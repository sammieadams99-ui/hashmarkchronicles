...
async function fetchPlayerStats(id) {
  ...
  try {
    ...
    return {
      ...
      int: interceptions,
      cmp_att:
        completions != null && attempts != null ? `${completions}/${attempts}` : null
    };
  } catch (e) {
...
async function main() {
  try {
    ...
    if (defaultFeatured) {
      ...
      const fetchId = Number(featured.id);
      const liveStats = Number.isFinite(fetchId) ? await fetchPlayerStats(fetchId) : null;
