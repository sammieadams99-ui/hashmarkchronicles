...
if (!CFBD_KEY) {
  console.error("[cfbd] Missing CFBD_KEY env var — aborting.");
  process.exit(1);
}
...
async function run() {
  ...
  if (!latest) {
    console.log("No completed game found — skipping spotlight build (exit 0).");
    process.exit(0);
  }
...
run().catch((err) => {
  console.error("[cfbd] Non-fatal build error:", err);
  process.exit(0);
});
