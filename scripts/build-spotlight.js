...
function splitBySide(list,mapRaw){ const O=[],D=[]; list.forEach(p=>{ const mp=(mapRaw[p.name]?.pos)||p.position; (sideOf(p.position,mp)==='OFF'?O:D).push(p);}); return[O,D]; }
function score(list){
  return list.map(p=>{
    ...
    const passINT=toN(p.interceptionsThrown||p.interceptions),fum=toN(p.fumblesLost);
    const tkl=toN(p.tackles||p.totalTackles),tfl=toN(p.tfl||p.tacklesForLoss),sacks=toN(p.sacks),
          ints=toN(p.defInterceptions||p.interceptionsDef),pbu=toN(p.passesDefended||p.passesBrokenUp),
          ff=toN(p.forcedFumbles),fr=toN(p.fumblesRecovered),dtd=toN(p.defensiveTD||p.defTD);
    const off = passY*0.04+(rushY+recY)*0.10+passTD*4+(rushTD+recTD)*6-passINT*2-fum*2;
    const def = tkl*0.5+tfl*1+sacks*2+ints*3+pbu*0.5+ff*2+fr*1+dtd*6;
    return {...p, score: off+def};
  });
}
function compact(p){
  const out={};
...
function toCardLast(idx,lastOpp,seasonIdx){ return p=>{
  const meta=resolveMeta(idx,p.name);
  const id=(meta.id||'').toString();
  const pos=p.position||meta.pos||'';
  const slugStr=meta.slug||slug(p.name);
  const headshot=id?`https://a.espncdn.com/i/headshots/college-football/players/full/${id}.png`:'';
  const espn=id?`https://www.espn.com/college-football/player/_/id/${id}/${slugStr}`:`https://www.espn.com/search/results?q=${encodeURIComponent(p.name+' Kentucky football')}`;
  const seasonObj=seasonIdx.get(norm(p.name))||{};
  return { name:p.name,pos,slug:slugStr,headshot,espn, last_game:Object.assign(lastOpp?{opp:lastOpp}:{},compact(p)), season:compact(seasonObj) };
};}
function toCardSeason(idx){ return p=>{
  const meta=resolveMeta(idx,p.name);
  const id=(meta.id||'').toString();
  const pos=p.position||meta.pos||'';
  const slugStr=meta.slug||slug(p.name);
  const headshot=id?`https://a.espncdn.com/i/headshots/college-football/players/full/${id}.png`:'';
  const espn=id?`https://www.espn.com/college-football/player/_/id/${id}/${slugStr}`:`https://www.espn.com/search/results?q=${encodeURIComponent(p.name+' Kentucky football')}`;
  return { name:p.name,pos,slug:slugStr,headshot,espn, last_game:{}, season:compact(p) };
};}

function aggregateGame(rows){
  const by=new Map();
...
const top3 = arr => arr.sort((a,b)=>b.score-a.score).slice(0,3);
