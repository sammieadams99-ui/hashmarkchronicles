...
async function main() {
  ...
  const pass = by('passing').sort((a,b) => (b.passing_yds||0) - (a.passing_yds||0))[0] || {};
  const rush = by('rushing').sort((a,b) => (b.rushing_yds||0) - (a.rushing_yds||0))[0] || {};
  const recv = by('receiving').sort((a,b) => (b.receiving_yds||0) - (a.receiving_yds||0))[0] || {};
  ...
  const items = [
    ...
    { label: 'Off PPA/play', unit: '', val: (seasonAdv.off_ppa ?? null)?.toFixed?.(2), last: (lastAdv?.off_ppa ?? null)?.toFixed?.(2), ...dir(seasonAdv.off_ppa, lastAdv?.off_ppa) },
    ...
    { label: (pass.player || 'QB'), unit: 'yd', val: compactNumber(pass.passing_yds), last: null, d: 'steady' },
    { label: (rush.player || 'RB'), unit: 'rush', val: compactNumber(rush.rushing_yds), last: null, d: 'steady' },
    { label: (recv.player || 'WR'), unit: 'rec', val: compactNumber(recv.receiving_yds), last: null, d: 'steady' },
